...
def basic_preprocessing(_D):
    ...
    # # fix 'correct' to be boolean
    # _D['correct'] = _D['correct'].astype(int)
    return _D
...
def apply_exclusion_criteria(D, familiarization_D=None, verbose=False):
    ...
    D_trials = D[(D['condition'] == 'prediction') & (D['trial_type'] == 'video-overlay-button-response')]
    ...
    streakyIDs = []
    for name, group in D_trials.groupby(userIDcol):
    ...
    Dacc = D_trials[D_trials['correct'].isna() == False]
    ...
    Drt = D_trials.groupby(userIDcol).agg({'logRT':np.median})
    ...
    if verbose:
        print('There are a total of {} valid sessions for {}.'.format(D[userIDcol].nunique(), scenarionName))   
    return D
