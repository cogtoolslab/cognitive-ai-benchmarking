...
*/
  ...
  var choices = ['Fine 👍', 'Occluded 😶‍🌫️', 'Miraculous 😮', 'Weird/Broken 🤔', 'Overlay wrong 🎭', 'Other/Multiple 🤷‍♂️']
  ...
  function Experiment() {
    ...
    this.response_allowed_while_playing = true;
    // this.phase = 'experiment';
    this.condition = "prediction";
    this.prompt = "Is this stimulus acceptable?";
  ...
  const includeFamiliarizationTrials = false;
  ...
  var familiarization_trials_pre = _.map(
    ...
    function (n, i) {
      return _.extend({}, familiarizationExperimentInstance, n, {
        ...
        overlay_time: 1,
        blink_time: 500,
        stop: 30, //STIM DURATION stop the video after X seconds
        width: 500,
        height: 500,
        post_trial_gap: 0,
        on_start: stim_log,
      ...
      return _.extend({}, familiarizationExperimentInstance, n, {
        ...
        prompt: () => {
          if (last_correct & last_yes) {
            return "✅ Nice, you got that right. The red object did indeed hit the yellow area. Above, you see the full video again.";
          } else if (last_correct & !last_yes) {
            return "✅ Nice, you got that right. The red object indeed did not hit the yellow area. Above, you see the full video again.";
          } else if (!last_correct & last_yes) {
            return "❌ Sorry, you got that one wrong. The red object did not hit the yellow area. Above, you see the full video again.";
          } else {
            return "❌ Sorry, you got that one wrong. The red object did hit the yellow area. Above, you see the full video again.";
          }
        },
        // save_trial_parameters: {} //selectively save parameters
      });
  ...
  var instructionsHTML = {
    str1: [
      "<p> On each trial, you will see a brief video of a few objects interacting.</p><p>Your task will be to determine whether \
    a certain event happened.</p><p>Before the video starts, you'll see one object flash in red and an area in yellow. Remember these objects! You'll be asked if the object marked in red touched the area marked in yellow.\
  ...
  var exitSurveyText = _.extend({}, surveyTextInfo, {
    ...
    questions: [
      {
        prompt: "What strategies did you use to determine what happened?",
        rows: 5,
        name: "strategies",
        columns: 40,
      },
